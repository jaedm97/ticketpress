...
if ( ! class_exists( 'WP_Settings' ) ) {
	class WP_Settings {
		...
		function generate_media( $option ) {
			...
			$_mime_type = isset( $media_post->post_mime_type ) ? $media_post->post_mime_type : '';
			$_mime_type = explode( '/', $_mime_type );
			$mime_type  = isset( $_mime_type[1] ) ? $_mime_type[1] : '';

			wp_enqueue_media();

			?>
            <div class="media_preview <?php echo esc_attr( $mime_type ); ?>" style="width: 150px;margin-bottom: 10px;background: #d2d2d2;padding: 15px 5px;text-align: center;border-radius: 5px;">
                ...
                jQuery(document).ready(function ($) {
                    ...
                    $(document).on('click', '#media_upload_<?php echo esc_attr( $field_id ); ?>', function () {
                        ...
                        wp.media.editor.send.attachment = function (props, attachment) {
                            ...
                            $("#media_preview_<?php echo esc_attr( $field_id ); ?>").parent().removeClass().addClass('media_preview').addClass(attachment.subtype);
		...
		function generate_file( $option ) {
			...
			?>
            <input type="file" <?php echo esc_attr( $disabled ); ?> <?php echo esc_attr( $multiple ); ?> <?php echo esc_attr( $required ); ?>
                   name="<?php echo esc_attr( $id ); ?>"/>


            <div class="media_preview <?php echo esc_attr( $file_ext ); ?>">
                <img id="media_preview_<?php echo esc_attr( $field_id ); ?>" src="<?php echo esc_url( $value ); ?>"/>
            </div>
			<?php
		}
...
//			$post          = get_post( $post_id );
		...
		function generate_select( $option ) {
            ...
            <script>
                jQuery(document).ready(function () {
                    jQuery('#<?php echo esc_attr( $id ); ?>').niceSelect();
                });
            </script>
			<?php
		}
